...
def viewTickets():
    if 'loggedin' in session:
        ...
        while dictionary != False:
            ...
            dictionary = ibm_db.fetch_both(stmt)
        if tickets:
            return render_template("viewTicketsRaisedAdmin.html", tickets = tickets, username = session['ADMINUSERNAME'])
    return redirect(url_for('adminlog'))
...
def assignTicketsAgentAdmin(name):
    if 'loggedin' in session:
        ...
        while dictionary != False:
            ...
            dictionary = ibm_db.fetch_both(stmt)
        if agents:
            return render_template("assignAgentassiAdmin.html", agents = agents, username = session['ADMINUSERNAME'],ticketid=name)
    return redirect(url_for('adminlog'))
...
def assignTicketsAdmin(ticket, username):
    if 'loggedin' in session:
        msg=""
        stmt = "UPDATE TICKET SET STATUS = ?, AGENTUSERNAME = ? WHERE TICKETID = ?"
        prep_stmt = ibm_db.prepare(conn, stmt)
        ...
        while dictionary != False:
            ...
            dictionary = ibm_db.fetch_both(stmt)
        if agents:
            return render_template("assignAgentAdmin.html", agents = agents, username = session['ADMINUSERNAME'],ticketid=ticket, msg="Agent alloted successfully.")
    return redirect(url_for('adminlog'))
